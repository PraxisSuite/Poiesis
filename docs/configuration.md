...
vm:
  ...
  cpu_type: x86-64-v2-AES         # Default VM cpu model; per-deploy override available via the `cpu_type` field in the deployment JSON. RHEL 10 family (Rocky/Alma 10) needs `x86-64-v3` — auto-resolved from cloud-images.yaml's `cpu_baseline` field, no override needed. Use `kvm64` for max compatibility across older hardware.
