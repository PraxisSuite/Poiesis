...
| `cpu_type` | optional | — | Per-deployment override for the QEMU `-cpu` model. Resolution order (highest wins): (1) this field, (2) the image catalog's `cpu_baseline` for the chosen image (auto-resolves RHEL 10 family to `x86-64-v3`), (3) `vm.cpu_type` from `config.yaml` (default `x86-64-v2-AES`). So Rocky 10 / AlmaLinux 10 / CentOS Stream 10 are handled automatically — set this only to override the catalog default. Common values: `x86-64-v2-AES` (default), `x86-64-v3`, `x86-64-v4`, `host`, or a specific CPU model name (`Haswell`, `Skylake`, …). The preflight `check_node_cpu_baseline` verifies the target node's CPU has every flag the requested type needs and fails fast with a clear error if it doesn't. See [BUG-006 in known-bugs.md](../../known-bugs.md) for the full background. |
