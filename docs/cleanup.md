...
Each resource gets one of three actions:
...
| `--list-file FILE` | Load a pre-built JSON action list. See [--list-file Usage](#--list-file-usage). |
...
The three possible values for the `action` field and their effects:
...
After all actions complete, a summary panel is printed with five possible outcomes/states:
