...
def load_config() -> dict:
    ...
    if not config_path.exists():
        ...
        console.print("Copy config.yaml from the repo and fill in your credentials.")
        sys.exit(1)
...
def run_ansible_add_dns(cfg: dict, hostname: str, container_ip: str) -> None:
    """Register A and PTR records on the BIND DNS server."""
    dns = cfg.get("dns", {})
    ...
    # Derive reverse zone file from IP: 10.20.20.140 -> 20.20.10.in-addr.arpa.hosts
    ...
    console.print(f"  [dim]Registering {fqdn} → {container_ip} on {dns['server']}...[/dim]")
    result = subprocess.run(cmd, cwd=str(ansible_dir))
    if result.returncode != 0:
        console.print(f"  [yellow]Warning: DNS registration failed. "
                      f"Add manually: {fqdn} A {container_ip} and PTR.[/yellow]")
    else:
        console.print(f"  [green]✓ DNS registered: {fqdn} → {container_ip} (+ PTR)[/green]")


def run_ansible_inventory_update(cfg: dict, hostname: str, container_ip: str, password: str) -> None:
    """Run the inventory-update playbook against the development server."""
    inv_cfg = cfg["ansible_inventory"]
    ansible_dir = Path(__file__).parent / "ansible"
    ...
    if result.returncode != 0:
        ...
        console.print(f"  [yellow]Warning: Inventory update on {dev_server} failed. "
                      f"Add manually: {hostname} ansible_host={container_ip} to [{inv_cfg['group']}][/yellow]")
    else:
